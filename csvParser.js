...
function parseTransaction(line, lineNumber, filename) { // If transaction is valid, return transaction object. If transaction not valid, throw error.
    if (!moment(line[0], "DD-MM-YYYY").isValid()) {
        ...
        return new transactionHandler.Transaction(moment(line[0], "DD-MM-YYYY"), line[1], line[2], line[3], Number(line[4]));
    }
}

function parseCsv(rawData, filename) { // Takes in raw CSV data, checks for validity, logs any errors and discards faulty transactions, returning an array of valid Transaction objects.
    const allTransactions = rawData.split('\r\n') // Splits raw data into an array of lines
        ...
        .map((line, index) => {
            try {
                return parseTransaction(line, index + 2, filename);
            } catch (err) {
                logger.error(err);
